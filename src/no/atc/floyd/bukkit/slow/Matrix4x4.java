...
public class Matrix4x4 
{
  ...
  /**
   * Construct a 4x4 matrix as a copy of the specified matrix.
   * 
...
//    }
    ...
    for (int i=0; i<16; i+=4) {
        for (int j=0; j<4; j++) {
        	product.m_[i+j] = matrix.m_[i]*m_[j] + matrix.m_[i+1]*m_[j+4] + matrix.m_[i+2]*m_[j+8] + matrix.m_[i+3]*m_[j+12];
        }
    }
  ...
  /**
   * Multiply this 4x4 matrix with the specified vector4.
   * 
  ...
  /**
   * Multiply this 4x4 matrix with the specified Bukkit vector.
   * 
   * @param vector   3-component Bukkit vector to multiply with.
   * @return         Result of operation.
   */
  public Vector transform (Vector vector)
  {
	    ...
	    result.setX(vector.getX() * m_[0]  +
	                vector.getY() * m_[4]  +
	                vector.getZ() * m_[8]  + m_[12]);

	    result.setY(vector.getX() * m_[1]  +
                    vector.getY() * m_[5]  +
                    vector.getZ() * m_[9]  + m_[13]);

	    result.setZ(vector.getX() * m_[2]  +
                    vector.getY() * m_[6]  +
                    vector.getZ() * m_[10]  + m_[14]);
